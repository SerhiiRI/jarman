...
*** TOREVIEW Table.clj [3/4]                                         :@Aleks:
    - [X] remove 1pixel black border for right table.
    - [ ] (expand-panel?) text "hide" replace on julia (not created at that moment) left arrow, and background fill with one gray color, as "tab", or left section in table.clj, where components is.
