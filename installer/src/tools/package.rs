...
pub mod FTP {
    ...
    pub fn ftp_file_list() -> Vec<String> {
    ...
    pub fn ftp_get_all_packages(repositories: &[&str]) -> Vec<PandaPackage> {
        let mut packagelist: Vec<PandaPackage> = Vec::new();
        for repo_url in repositories {
            if let Ok(mut ftp_stream) = FtpStream::connect(repo_url) {
                if let Ok(_s) = &ftp_stream.login(FTP_LOGIN, FTP_PASSWORD) {
                    for ftp_file in &ftp_stream.nlst(Some(PROGRAM.to_lowercase().as_str())).unwrap() {
    ...
    pub fn ftp_download_package(package: PandaPackage) -> Result<(), String> {
        match package {
            PandaPackage { file: Some(file_name), name, version, artifacts, uri: Some(url) } => {
                if let mut ftp_stream = FtpStream::connect(url).unwrap() {
                    if let Ok(_some) = &ftp_stream.login(FTP_LOGIN, FTP_PASSWORD) {
                        if let _01TMP = &ftp_stream.cwd(PROGRAM.to_lowercase().as_str()) {
                            let mut buff = Vec::new();
...
pub(crate) fn version_resolver(packages:&[PandaPackage]) -> Option<PandaPackage> {
    match packages.len() {
        0 => return None,
