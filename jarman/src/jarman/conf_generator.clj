...
(defn hor-config-panel [l component]
  (mig [[(label :text (transform-label-to-name l))]
        [component]]
       :args [
              :constraints ["" "5%[45%, grow,right]10px[44%, grow, fill, left]5%" "0px[]0px"]
  ...
  ([component-vec border-string toolbox]
   (mig (concat 
         (vec (map vector (filter some? [(label :text (transform-label-to-name border-string)
                                                ...
                                                :border (line-border :top 2 :color (style :color_border)))
                                         toolbox])))
         component-vec) 
...
(defn- doc-map
  ...
  (let [{:keys [space header parameters description]} m
        ui-components (vec (map vector (filter some? [(if space (text :enabled? false
                                                                      :multi-line? true
                                                                      :wrap-lines? true 
                                                                      :background (style :color_bg)
                                                                      :border (compound-border (empty-border :top 50) (line-border :bottom 2 :color "#000"))))
                                                      (if header (text :halign :left
                                                                       ...
                                                                       :border (empty-border :left 50 :right 50)
                                                                       :background (style :color_bg)))
                                                      (if description (mig [[(text :text (lang :description)
                                                                                   :multi-line? true
                                                                                   :wrap-lines? true
                                                                                   :background (style :color_bg)
                                                      ...
                                                      (if parameters (mig [[(text :text (lang :parameters)
                                                                                  :multi-line? true
                                                                                  :wrap-lines? true
                                                                                  :font (style :font_bold)
                                                                                  :background (style :color_bg)
                               ...
                               {:icon icon/refresh-blue-64-png
                                ...
                                (fn [e]
                                  ...
                                  (if (and (config-saver #(get-in % key-path)) (datamanager/test-connection (config-saver #(get-in % key-path))))
                                    (do (alert (lang :info_connection_suc) :title (lang :title_information) :type :info :icon (image-scale icon/agree-64-png 50)))
                                    (do (alert (lang :info_connection_fal) :title (lang :title_information) :type :info :icon (image-scale icon/X-64x64-png 50))))
                                  )}})]]
  ...
  [txt file] (let [text-area (text :multi-line? true
          ...
          (mig [[(mig [[(label :text (str file)
                      ...
                      ;;  [text-font-combobox] [up-text] [down-text] ;; [search-in-text]
                       [up-text] [down-text] ;; [search-in-text]
                       ]
    ...
    (condp in? extension
      ["clj" "edn"] (do (dosync (ref-set config (into (sorted-map) (clojure.edn/read-string (slurp file)))))
                        (mig [[(scrollable (clojure-map-form config) :border nil)]
                              [(mig [[(declare-button-list {(lang :btn_save)  {:function (fn [e] (if (dlg (lang :save_changes?) (lang :title_information) (fn [x] true) (fn [x] nil))
                                                                                                   (do (spit file (prn-str @config))
                                                                                                       (alert (lang :info_changes_saved_and_reload) :title (lang :title_information) :type :info :icon (image-scale icon/agree-64-png 60)))))
      ...
      (do (dosync (ref-set config (slurp file)))
          (mig [[(scrollable (simple-text-editor config file) :border nil)]
                [(declare-button-list {(lang :btn_save)  {:function (fn [e] (if (dlg (lang :save_changes?) (lang :title_information) (fn [x] true) (fn [x] nil))
                                                                             (do (spit file @config)
                                                                                 (alert (lang :info_changes_saved) :title (lang :title_information) :type :info :icon (image-scale icon/agree-64-png 60)))))
                                                          :icon icon/agree-64-png}})]]
               :args [:constraints ["wrap 1" "0px[grow, fill]0px" "0px[grow, fill]0px[30, fill]0px"]])))))
...
(UIManager/put "Tree.collapsedIcon" (image-scale icon/down-grey1-rotate90-64x64-png 15))
(UIManager/put "Tree.expandedIcon" (image-scale icon/down-grey1-64-png 15))
  ...
  (proxy [javax.swing.tree.DefaultTreeCellRenderer] []
    (^java.awt.Component getTreeCellRendererComponent [^javax.swing.JTree tree        ^java.io.File      value 
     ...
     (let [icon-by-extension (fn [file-extension] 
       ...
       (if leaf
         (mig [[(label :text (config-name (.getName (cast java.io.File node-value)))
                       :icon (image-scale (icon-by-extension (extension (str node-value))) 30)
                       :background "#FFF"
                       )]]
              :args [:background "#fff"])
         (if expanded
           (label :text (.getName (cast java.io.File node-value))
...
(defn create-tree []
  (if (.isFile (io/file cfg/*configuration-path*))
    (mig [[(label :text (str (style :config_to_file) (.getName (io/file cfg/*configuration-path*)))
                  :font {:style :bold}
                  :background (style :color_bg)
                  :border (compound-border (line-border :thickness 10 :color (style :color_bg)) (line-border :bottom 2 :color (style :color_border)))
                  :halign :center)]
          [(view-by-extention (io/file cfg/*configuration-path*) (ref nil))]]
         :args [:constraints ["wrap 1" "0px[grow, fill]0px" "0px[]0px"]
                ])
    (let [config (ref nil)
      ...
      (left-right-split (scrollable tree-view :background "#fff" :border (empty-border :thickness 10 :color "#fff"))
                        v-panel
...
(defn config-frame []
  (binding [cfg/*configuration-path* (str cfg/*configuration-path* "/database.edn")]
    (frame :title "Mr. Jarman - Config Tool"
           :content (create-tree))))
