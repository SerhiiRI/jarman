(ns jarman.config.tree-console-view
  (:gen-class)
  ...
  (if-not (some #(= \. %) (seq file-name)) nil
          (let [frmt (last (clojure.string/split file-name #"\."))]
            (condp in? frmt
              ...
              *prop-files*      "shell"
              "unsupported file"))))
(clojure.string/split "src/jarman/config" #"/")
(map println (sort-by #(.isDirectory %) (.listFiles (clojure.java.io/file "./"))))
