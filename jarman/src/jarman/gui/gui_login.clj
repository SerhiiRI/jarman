...
(def confgen-header
  (fn [title] (c/label :text title :font (gtool/getFont 16 :bold)
...
(def confgen-title
  (fn [title]
    (c/label :text title :foreground light-grey-color :font (gtool/getFont 12 :bold))))

(def confgen-input 
  (fn [data] (c/text :text data :font (gtool/getFont 14)
               :background "#fff"
               :columns 20
...
(defn validate-fields
  ...
  (if (= 0 (+ (if (= (c/text v-dbtype) "") (do (c/config! v-dbtype :border (color-border red-color)) 1)
                  (do (c/config! v-dbtype :border (color-border blue-green-color)) 0))
              (if (< (count (c/text v-host)) 4) (do (c/config! v-host :border (color-border red-color)) 1)
                  (do (c/config! v-host :border (color-border blue-green-color)) 0))
              (if (= (re-find #"[\d]+" (c/text v-port)) nil)
                (do (c/config! v-port :border (color-border red-color)) 1)
                (if (< (Integer. (c/text v-port)) 65000)
                  (do (c/config! v-port :border (color-border blue-green-color)) 0)
                  (do (c/config! v-port :border (color-border red-color)) 1)))
              (if (=  (c/text v-dbname) "") (do (c/config! v-dbname :border (color-border red-color)) 1)
                  (do (c/config! v-dbname :border (color-border blue-green-color)) 0))
              (if (=  (c/text v-user) "") (do (c/config! v-user :border (color-border red-color)) 1)
                  (do (c/config! v-user :border (color-border blue-green-color)) 0))
              (if (=  (c/text v-password) "") (do (c/config! v-password :border (color-border red-color)) 1)
...
(defn config-generator-fields [state! dispatch! key-title]
  (if (nil? (conn/datalist-get))
    ...
    (let [data (key-title (conn/datalist-get))
          ...
          err-lbl (c/label :text "" :foreground blue-green-color :font (myFont 14))
          btn-del (c/label :text "Delete" :background "#fff"
                         :class :css1
                         :border (empty-border :top 10 :right 10 :left 10 :bottom 10)
                         :listen [:mouse-entered (fn [e] (c/config! e :background back-color :cursor :hand))
          ...
          btn-conn (c/label :text "Connect" :background "#fff"
                          ...
                          :listen [:mouse-entered (fn [e] (c/config! e :background back-color :cursor :hand))
                                   ...
                                   :mouse-clicked (fn [e] (if (= nil (conn/test-connection {:dbtype (c/text dbtype-inp)
                                                            ...
                                                            (c/config! err-lbl :text "ERROR!! PLEASE RECONNECT" :foreground red-color)
                                                            (c/config! err-lbl :text "SUCCESS" :foreground blue-green-color)))])
                                                     ...
                                                     :items [[(c/vertical-panel
                                                               :items (concat (list
                                                                               ...
                                                                               (c/label :text (gtool/htmling "<h2>FAQ</h2>")
                                                                                      :foreground blue-green-color :font (myFont 14)))
                                                                              (flatten (map (fn [x] [(c/label :text (str "- " (:q x)) :foreground blue-green-color :font (myFont 14) :border (empty-border :top 4 :bottom 4))
                                                                                                     (c/label :text (:a x) :foreground light-grey-color :font (myFont 14) :border (empty-border :top 4 :bottom 4))]) faq)))) :north]])
...
(defn some-error-v [error]
  (c/vertical-panel
   :background "#fff"
   :items (list (c/label :text error :font (myFont 13) :foreground red-color
                       :border (empty-border :top 5 :left 5 :bottom 5)))))


(defn label-to-config [state! dispatch! dbname title key-title login pass] 
  (let [dbn  (c/label :text dbname :font (myFont 15) :foreground blue-color
                                                          :border (empty-border :top 5 :left 5 :bottom 5))
        hostn (c/label :text title :font (myFont 12) :foreground light-grey-color
                                                          :border (empty-border :top 5 :left 5 :bottom 5)
        ...
        my-panel (c/border-panel
                  :listen [:focus-gained (fn [e] (c/config! e :border (line-border :bottom 4 :color light-blue-color)))
                           :focus-lost   (fn [e] (c/config! e :border (line-border :bottom 4 :color light-grey-color)))]
                  :focusable? true
    ...
    (let [onClick (fn [e] (let [data-log (test-key-login key-title (c/text login)
                            ...
                            (if (instance? clojure.lang.PersistentArrayMap data-log)
                              (do (.revalidate my-panel)
                                  ...
                                  (state/state :startup))
                              (do
                                (c/config! dbn :foreground red-color)
                                ...
                                (c/config! my-panel
                                         ...
                                         :listen [:mouse-clicked (fn [e]
                                                  ...
                                                  :mouse-exited  (fn [e] (do
                                                                           ...
                                                                           (c/config! e :border (line-border :bottom 4 :color red-color))))
                                                  :mouse-entered  (fn [e] (do (c/config! icon-conf :visible? true)
                                                                              (c/config! icon-info :visible? true)
                                                                              (c/config! e :border (line-border :bottom 4 :color red-color))))])
                                (.revalidate my-panel)))))]
      (c/config! my-panel :listen [:mouse-entered (fn [e] (do
                                                          (c/config! e :border (line-border :bottom 4 :color light-blue-color))
                                                          (c/config! icon-conf :visible? true)))
                                 :mouse-exited  (fn [e] (do (c/config! e :border (line-border :bottom 4 :color light-grey-color))
                                                            (c/config! icon-conf :visible? false)))
...
(defn- access-panels-list [state! dispatch!]
  (let [mig (mig-panel :constraints ["wrap 4" "20px[ left]20px" "20px[]20px"])
    ...
    (doall (map (fn [[k v]]
                  (.add mig (label-to-config state! dispatch!
                                             (:dbname v)
                                             (:host v)
                                             k
                                             (:login (state!))
    ...
    (.add mig
          (c/border-panel
           :center (c/label
                    ...
                    :halign :center
                    :listen [:mouse-clicked (fn [e] (c/config! (c/to-frame e) :content (config-generator-panel state! dispatch! :empty)))])
           :border (line-border :bottom 4 :color light-grey-color)
           :maximum-size    [120 :by 120]
           :preferred-size  [120 :by 120]
           :background      "#fff"))
...
(defn- error-panel [errors]
  (let [scr (c/scrollable
             (mig-panel
              ...
              :items [[(c/label :icon (stool/image-scale icon/alert-red-512-png 8)
                              :border (empty-border :right 10)) "split 2"]
                      [(c/label :text (gtool/htmling "<h2>ERROR</h2>") :foreground blue-green-color :font (myFont 14))]
                      [(let [mig (mig-panel
                                  :constraints ["wrap 1" "40px[:600, grow, center]40px" "10px[]10px"]
                                  :items [[(c/label :text (gtool/htmling "<h2>About</h2>")
                                                  :foreground blue-green-color :font (myFont 14)) "align l"]
                                          [(c/label :text (gtool/htmling (str "<p align= \"justify\">" (:description errors) "</p>"))
                                                  :foreground light-grey-color
...
(defn asks-panel [faq]
  (let [mig
        (mig-panel
         ...
         :items [])]
    (doall (map (fn [x] (do (.add mig (c/label :text (str "- " (:q x)) :foreground blue-green-color :font (myFont 14)) "align l")
                            (.add mig (c/label :text (:a x) :foreground light-grey-color :font (myFont 14)) "align l"))) faq))
    (.repaint mig)
    [(c/label :text (gtool/htmling "<h2>FAQ</h2>")
            :foreground blue-green-color
            :font (myFont 14)) mig]))
...
(defn info-panel [state! dispatch!]
  (let [info some-text
        faq [{:q "Why i can not get connection with server?"
              :a "Please check that you have entered the data correctly"}
             {:q "Where can i find data to connect?"
        ...
        scr (c/scrollable
             (mig-panel
              ...
              :items [[(c/label :icon (stool/image-scale icon/left-blue-64-png 50)
                      ...
                      [(c/label :icon (stool/image-scale "icons/imgs/trashpanda2-stars-blue-1024.png" 47))]
                      [(let [mig (mig-panel
                                  :constraints ["wrap 1" "100px[:600, grow, center]100px" "20px[]20px"]
                                  :items [[(c/label :text (gtool/htmling "<h2>About</h2>")
                                                  :foreground blue-green-color :font (myFont 14)) "align l"]
                                          [(info  light-grey-color)]
                                          [(c/label :text (gtool/htmling "<h2>Jarman</h2>")
                                                  :foreground blue-green-color :font (myFont 14)) "align l"]
                                          [(info  light-grey-color)]
                                          [(c/label :text (gtool/htmling "<h2>Contact</h2>")
                                                  :foreground blue-green-color :font (myFont 14)) "align l"]
                                          [(info  light-grey-color)]
                                          [(c/label :text (gtool/htmling "<h2>Links</h2>")
                                                  :foreground blue-green-color :font (myFont 14)) "align l"] 
                                          [(c/label :text (gtool/htmling "<p align= \"justify\">http://trashpanda-team.ddns.net</p>")
...
(defn login-panel 
  "Description:
     Build and return form for login panel.
  Example:
   ...
   :items (gtool/join-mig-items
           ...
           (gcomp/vmig ;; Main content
            ...
            :items (gtool/join-mig-items
                    ...
                    (access-panels-list ;; configs list
                     state! dispatch!)))
           
           (gcomp/hmig ;; More info buttons
            :hrules "[grow, right]"
            :items [[(c/label :icon (stool/image-scale icon/a-blue-64-png 40)
                              :listen [:mouse-clicked (fn [e]
                                                        (c/config!
                                                         (c/to-frame e)
