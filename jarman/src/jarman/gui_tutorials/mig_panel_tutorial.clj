...
(def mig (mig-panel
          :constraints ["wrap 2" "10px[grow, center]5px[grow, fill]10px" "10px[]0px"]
          ;; definition of appearance some like flex in css
          :border (line-border :thickness 3 :color "#f00")
          ;; border around first mig-panel
          :items [ ;; items vector
                  [(label :text "Tekst w labelu") "gaptop 10"]
                  ;; "gaptop 10" is a margin-top 10px for row
                  ["Po prostu tekst"]
                  [(label :text "Tekst w labelu zajmujący dwie kolumny" :background "#bca") "span 2"]
                  ;; span 2 mean use two cell as one
                  [(label :text "Element w split 2" :background "#abc") "split 2"]
                  ;; split column to two space for component
                  [(label :text "Drugi element w split 2" :background "#abc")]
                  ;; this label go to upper split 
                  [(mig-panel :constraints ["" "[grow, center]" ""]
                              ;; mig-panel as one component for first mig, but inside have two another component
                              :border (line-border :thickness 3 :color "#666")
                              ;; border around mig-panel inside migpanel
                              :items [;; items vector
                                      [(label :text "Element w mig 2" :background "#a9a")]
                                      ;; simple label
                                      [(label :text "Drugi element w mig 2" :background "#a9a")]
                                      ;; simple label
                                      ])]])
