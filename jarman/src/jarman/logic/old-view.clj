...
(quick-path :point_of_sale_group_links)
;; {:tbl "point_of_sale_group_links",
;;  :ref [{:tbl "point_of_sale_group", :ref nil}
;;        {:tbl "point_of_sale", :ref [{:tbl "enterpreneur", :ref nil}]}]}
...
(defn construct-sql [table select-rules]
  ...
  (let [m (first (mt/getset table))
    ...
    {:update (fn [entity] (if (or (id_column entity) (:id entity)) (update table-name :set entity :where (=-v id_column (id_column entity)))))
     :insert (fn [entity] (insert table-name :values (vals entity)))
     :delete (fn [entity] (if (or (id_column entity) (:id entity)) (delete table-name :where (=-v id_column (id_column entity)))))
     :select (fn [& {:as args}]
...
(defmacro defview [table & {:as args}]
  (let [stable (make-name (str table) "-view")]
    `(let [config#          (atom (assoc ~args :table-name (keyword '~table)))
           ...
           model#     (gui-table-model-columns (:tables @config#) (:view @config#))
           table#     (gui-table (gui-table-model model# data#))
...
;; (do
;;   (load-file "./src/jarman/logic/modul_a.clj")
;;   (println *ns*))


;; +----------------------------+----------------------+-----------------------+----------------------+-----------------------+
;; | (date-edit-menu)           |(data-edit-right)     |                       |                      |                       |
;; |                            +----------------------+-----------------------+----------------------+-----------------------+
;; |                            |                      |                       |                      |                       |
;; |                            |                      |                       |                      |                       |
;; |                            |                      |                       |                      |                       |
...
;; (defn jarman-agenda [configuration data-toolkit]
;;   (println "Configuration atom:")
;;   (println "\t;;=>" (keys configuration))
;;   (println "Data toolkit:")
;;   (println "\t;;=>" (keys data-toolkit)))

;; (defn jarman-csv-exporter [configuration data-toolkit]
;;   (println "Configuration atom:")
;;   (println "\t;;=>" (keys configuration))
;;   (println "Data toolkit:")
;;   (println "\t;;=>" (keys data-toolkit)))

;; (defn jarman-calendar [configuration data-toolkit]
;;   (println "Configuration atom:")
;;   (println "\t;;=>" (keys configuration))
;;   (println "Data toolkit:")
;;   (println "\t;;=>" (keys data-toolkit)))

;; (defmacro defview [configuration]
;;   (let [config          (atom (assoc ~args :table-name (keyword '~table)))
;;         backup-config   (deref config)
;;         restore-config  (fn [] (reset! config backup-config))
;;         ktable    (:table-name @config)
...
;;         model     (gui-table-model-columns (:tables @config) (:view @config))
;;         table     (construct-table (gui-table-model model data))
...
(defmacro defview2 [table-model-name & body]
  (let [configurations
        (reduce into ;; {:name "Editable table"
                     ;;  :place "fdsa"
                     ;;  :tables [:permission]
...
(defview2 permission
  (jarman-table
   :name "Edit user per" ;; TODO
   :place "menu/data-edit-menu" ;; TODO
   :tables [:client_visit]
   :view [:client_visit.reason :client_visit.configuration]
   :query {:column (as-is :client_visit.id :client_visit.reason :client_visit.configuration)})
  (jarman-table
   :name "Edit permission" ;; TODO
   :place "menu/data-edit-menu" ;; TODO
   :tables [:client_visit]
   :view [:client_visit.reason :client_visit.configuration]
   :query {:column (as-is :client_visit.id :client_visit.reason :client_visit.configuration)})
  (jarman-csv-exporter
   :name "Exporter panel" ;; TODO
   :place "menu/data-edit-menu/bottom" ;; TODO
   :tables [:client_visit :client]
   :column [:client_visit.date_from :client.fname (processed-by :client.lname (fn [name] (format "<h1>%s</h1>" name)) (fn [name] (format "<div>%s</div>" name)))]
   :query {:inner-join [:client] :column (as-is :client_visit:date_from :client.fname :client.lname)})
  (jarman-agenda
   :place "menu/tab" ;; TODO
   :tables [:client :client_visit]
   :view [:client_visit.reason]
   :query {:column (as-is :client_visit.id :client_visit.reason :client.fname :client:lname)})
  (jarman-calendar
   :name "Calendar" ;; TODO
   :place "menu/data-edit-menu" ;; TODO
   :tables [:client_visit :client]
   :from-to [:client_visit.date_from :client_visit.date_to ]
   :color 10
   :information [:client_visit]
   :query {:column (as-is :client_visit.date_from :client_visit.date_to)
           :where [:=
                   [:<> :client_visit.date_from nil]
                   [:<> :client_visit.date_to   nil]]})
  ;; (jarman-unit-calendar2)
  )
...
;; jarman-table
;;   Make view to table 
;;   Invoked to  "menu/data-edit-menu"
;; jarman-csv-exporter
;;   Depend on table view 
;;   Invoked to  "menu/data-edit-menu/bottom "
