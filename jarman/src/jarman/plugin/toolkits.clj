...
#_(defn- tf-t-f [table-field]
  (let [t-f (string/split (name table-field) #"\.")]
    (mapv keyword t-f)))

#_(defn- t-f-tf [table field]
  (keyword (str (name table) "." (name (name field)))))

;;; Helper sql-functionality ;;; 
#_(defn- sql-operation-suffix [keyword-rule-name suffix] {:pre [(keyword? keyword-rule-name) (keyword? suffix)]}
  (string/ends-with? (name keyword-rule-name) (name suffix)))

#_(defn- sql-operation-expression [keyword-rule-name] {:pre [(keyword? keyword-rule-name)]}
  (keyword (format "%s-%s" (name keyword-rule-name) "expression")))

#_(defn- sql-make-select-wrapper
  "Description
...
#_(defn- supply-sql-action-constructor [configuration toolkit-map]
...
#_(defn- sql-crud-toolkit-constructor
  "Description
...
#_(defn- export-toolkit-constructor [configuration toolkit-map]
...
#_(defn- document-toolkit-constructor [configuration toolkit-map]
...
#_(defn data-toolkit-pipeline [configuration other-toolkit-map]
...
(defn jack-in-metadata-table [toolkit-map configuration]
  {:pre [(some? (:table_name configuration))]}
  (if-let [table-metadata (first (mt/return-metadata (:table_name configuration)))]
    (let [meta-obj (mt/->TableMetadata table-metadata)]
      (assoc toolkit-map :meta-obj meta-obj))
    toolkit-map))
